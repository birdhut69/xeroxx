    ...
    <>
      <header className="bg-[#075E54] text-white flex justify-between items-center h-[52px] sm:h-[60px] px-2 sm:px-6 w-full z-50 sticky top-0 border-b border-[#bec9c5]/30 shadow-md no-print select-none safe-bottom">
        ...
        <div className="flex items-center gap-1 sm:gap-3 min-w-0">
          ...
          >
            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-white/20 border border-white/30 flex items-center justify-center text-white shadow-xs shrink-0">
              <Shield className="w-4 h-4 sm:w-5 sm:h-5 fill-current" />
            </div>
            <span className="text-sm sm:text-lg font-bold tracking-tight text-white leading-none">
              CipherPrint
            </span>
          </div>

          {/* Mode Switcher Pills (Ultra-Compact on Mobile) */}
          <nav className="flex items-center bg-black/25 p-0.5 sm:p-1 rounded-xl border border-white/10 shrink-0">
            <button
              onClick={() => handleSelectMode('CUSTOMER')}
              className={`flex items-center gap-1 px-1.5 sm:px-3 py-1 rounded-lg text-[11px] sm:text-xs font-bold transition-all cursor-pointer ${
                currentMode === 'CUSTOMER'
                  ? 'bg-white text-[#075e54] shadow-sm'
                  : 'text-white/80 hover:text-white hover:bg-white/10'
              }`}
              title={t('customerMode')}
            >
              <Smartphone className="w-3.5 h-3.5 shrink-0" />
              <span className="hidden md:inline">{t('customerMode')}</span>
            </button>
            ...
            <button
              ...
              title={t('terminalMode')}
            >
              <Monitor className="w-3.5 h-3.5 shrink-0" />
              <span className="hidden md:inline">{t('terminalMode')}</span>
              {!isAdminAuthenticated && <KeyRound className="w-3 h-3 text-amber-300 ml-0.5 shrink-0" />}
            </button>
          </nav>
        </div>
        ...
        <div className="flex items-center gap-0.5 sm:gap-1.5 shrink-0">
          ...
          <div ref={langMenuRef} className="relative">
            <button
              ...
              className="flex items-center gap-1 px-1.5 sm:px-2.5 py-1 rounded-lg sm:rounded-xl bg-black/25 hover:bg-black/40 border border-white/15 text-xs font-bold text-white transition-colors cursor-pointer"
              title="Change Language (भाषा बदला)"
            >
              <span className="text-xs sm:text-sm">{currentLangObj.flag}</span>
              <span className="hidden lg:inline font-semibold text-xs">{currentLangObj.label}</span>
          ...
          <button
            ...
            className="bg-[#EF4444] hover:bg-red-600 text-white px-2 sm:px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 shadow-sm transition-transform active:scale-95 cursor-pointer"
            title={t('emergencyPurgeDesc')}
          >
            <Flame className="w-3.5 h-3.5 fill-current shrink-0" />
            <span className="hidden lg:inline">{t('emergencyPurge')}</span>
          </button>
          ...
          {/* Comparison Modal (Desktop) */}
          <button
            onClick={() => setShowComparison(true)}
            className="p-1.5 rounded-lg hover:bg-white/10 text-white transition-colors cursor-pointer hidden lg:block"
